...
export const EditCheckoutDate = props => {
  ...
  // state
  const [selectedCheckoutDate, setSelectedCheckoutDate] = useState(data?.checkoutDate ? dayjs(data.checkoutDate) : '')
  const [selectedCheckoutTime, setSelectedCheckoutTime] = useState(data?.checkoutDate ? dayjs(dayjs(data.checkoutDate).format('HH:mm:ss'), 'HH:mm:ss'): '')
  ...
  const onCheckoutChange = newCheckoutDate => {
    ...
    if(dayjs(newCheckoutDate).isAfter(dayjs(data.checkoutDate))) {
      let proposedNewDates = getArrayOfDatesBooked(dayjs(data.checkoutDate).add(1, 'day'), newCheckoutDate)
  ...
  const getAvailableRooms = () => {
    ...
    BookingsAPI.getRoomsByAvailability(data.checkinDate, selectedCheckoutDate).then(setAvailableRooms)
  }
    ...
    <Form 
      ...
      {showWarning && (
        <>
          ...
          <Row className="my-3">
            <Col span={12} align="left">
              <Button className="w-full" onClick={showChangeRoom}>Change Room</Button>
            </Col>
            <Col span={12} align="left">
      ...
      <Space>
        ...
        <Button className="mb-0" onClick={cancelEdit}>Cancel</Button>
