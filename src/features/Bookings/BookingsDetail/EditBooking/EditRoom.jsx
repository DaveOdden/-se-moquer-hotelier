...
export const EditRoom = props => {
  ...
  const [selectedRoom, setSelectedRoom] = useState({ key: data.room.roomNum.toString(), value: data.room.roomNum.toString(), id:data.room._id})
  ...
  const initialPopulateRoomDropdown = () => {
    if(data.checkinDate && data.checkoutDate ) {
      ...
      BookingsAPI.getRoomsByAvailability(data.checkinDate, data.checkoutDate).then((res) => {
  ...
  return (
    ...
    <Form 
      ...
      <Space>
        <Button className="mb-0" onClick={goBack}>Cancel</Button>
