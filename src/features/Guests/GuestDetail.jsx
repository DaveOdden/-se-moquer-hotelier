...
export const GuestDetail = (props) => {
	...
	const actionItems = [
		{
			key: 'edit',
			label: <Space size="large">Edit</Space>,
			icon: <EditOutlined />,
			onClick: () => setEditState(true),
		},
		{
			key: 'delete',
			label: <Space size="large">Delete</Space>,
			icon: <DeleteOutlined />,
			danger: true,
	...
	useEffect(() => {
		// loop over guest's raw data for content display
		if (guest) setDescriptionContent(transformDataForDescription(guest))
	}, [guest])

	useEffect(() => {
		// revert to read-only when form is updated
		if (formStatus.response) setEditState(false)
	}, [formStatus])
